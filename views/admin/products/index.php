...
<div id="editCategoryModal" class="fixed inset-0 hidden bg-slate-950/60 backdrop-blur-sm z-[100] flex items-center justify-center opacity-0 transition-opacity duration-300">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden transform scale-95 transition-all duration-300">
        ...
        <div class="bg-slate-900 p-6 text-center relative overflow-hidden">
            ...
            <button type="button" onclick="closeEditModal()" class="absolute top-4 right-4 text-slate-400 hover:text-white transition">
                <i class="ti ti-x text-xl"></i>
            </button>

            <div class="inline-flex items-center justify-center w-12 h-12 bg-blue-600 rounded-xl mb-3 shadow-lg shadow-blue-500/40 transform -rotate-6">
                <i class="ti ti-edit text-white text-2xl"></i>
            </div>
            <h2 class="text-xl font-bold text-white relative z-10">Mettre à jour <span class="text-blue-400">Catégorie</span></h2>
        </div>

        <form action="/admin/categories/update" method="POST" class="p-6 custom-form-tech space-y-5">
            <input type="hidden" name="id" id="edit-id">

            <div>
                <label class="text-[10px] uppercase tracking-[0.15em] font-black text-slate-400 mb-2 block">Nom de la catégorie</label>
            ...
            <div>
                <label class="text-[10px] uppercase tracking-[0.15em] font-black text-slate-400 mb-2 block">Description technique</label>
                <textarea name="description" id="edit-description" rows="3"
                          class="w-full bg-gray-50 border border-gray-100 p-3 rounded-xl focus:border-blue-600 focus:bg-white outline-none transition font-medium text-slate-600"
                          placeholder="Détails de la catégorie..."></textarea>
            </div>

            <div class="flex gap-3 pt-2">
                <button type="button" onclick="closeEditModal()"
                        class="flex-1 px-5 py-3 rounded-xl text-sm font-bold text-slate-400 hover:text-slate-600 hover:bg-gray-50 transition uppercase tracking-widest">
                    Annuler
                </button>
                <button type="submit"
                        class="flex-1 bg-blue-600 text-white px-5 py-3 rounded-xl text-sm font-bold shadow-lg shadow-blue-500/30 hover:bg-blue-700 hover:-translate-y-0.5 transition-all uppercase tracking-widest">
                    Enregistrer
                </button>
            </div>
        </form>
